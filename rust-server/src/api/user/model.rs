...
pub async fn _get_by_username(
    username: &str,
    db: &sqlx::MySqlPool,
    log: &slog::Logger,
) -> Result<Option<User>, error::Error> {
    crit!(log, "username: {}", username);
...
pub async fn insert(
    ...
    // let i: QueryAs<_, _, _> = sqlx::query_as(&sql);
    // Ok(())
...
pub async fn update(
    ...
    let mut sql1 = vec![format!("update_time = ?")];
    let mut sql_index = 2;
...
pub async fn delete(id: u64, db: &sqlx::MySqlPool, log: &slog::Logger) -> Result<(), error::Error> {
...
pub async fn get_user_info_by_id(
    ...
    let query = format!("SELECT id, username, create_time, mobile, last_login_time, last_login_ip, user_type FROM users
        WHERE id = {};",id.to_string());
    let r = sqlx::query_as::<_, UserInfo>(&query)
        .fetch_optional(db)
        .await;
