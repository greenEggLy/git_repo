...
pub fn route(cfg: &mut web::ServiceConfig) {
    cfg.service(create_auth);
    cfg.service(refresh_auth);
    cfg.service(delete_auth);
}
...
pub async fn create_auth(
    req_info: web::Json<CreateAuthReqJson>,
    ...
    let u = match result {
        None => {
            ...
            return Err(error::new(100400, "1", 422));
        }
        Some(v) => v,
    };

    let user_id = match u.id {
        None => return Err(error::new(100400, "2", 422)),
        Some(v) => v,
    };

    let pwd = match u.password {
        None => return Err(error::new(100400, "3", 422)),
        Some(v) => v,
    };
    if pwd != password {
        return Err(error::new(100400, "wrong pwd", 422));
    }

    let auth = auth::create_auth(user_id, u.user_type.unwrap(), &client, &state).await?;
...
pub async fn refresh_auth(
    ...
    let auth_id = claims.sub.parse::<u64>().unwrap();
