...
// 将用户登录的token加入黑名单
pub async fn insert_auth_black_list(
    auth_black_list: &AuthBlacklist,
    db: &sqlx::MySqlPool,
    log: &slog::Logger,
) -> Result<(), error::Error> {
    // let query = format!(
    ...
    //     VALUES ({}, {}, {});",
    //     auth_black_list.access_token_id, auth_black_list.access_token_exp, auth_black_list.user_id
    // );
    // let r = sqlx::query(&query).execute(db).await;
    let r = sqlx::query(
        r#"
        INSERT INTO authorizations_blacklist (access_token_id, access_token_exp, user_id)
        VALUES (?, ?, ?)"#,
...
pub async fn insert_auth(
    ...
    db: &sqlx::MySqlPool,
    log: &slog::Logger,
) -> Result<Authorization, error::Error> {
...
pub async fn get_by_id(
    ...
    db: &sqlx::MySqlPool,
    log: &slog::Logger,
) -> Result<Option<Authorization>, error::Error> {
    let r = sqlx::query_as::<_, Authorization>(
        r#"
        SELECT a.*
...
pub async fn update_auth(
    ...
    let sql = format!(
        "UPDATE authorizations SET {} WHERE id = ${}",
        sql1.join(","),
        sql_index
    );
