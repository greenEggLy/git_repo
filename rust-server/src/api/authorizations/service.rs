...
// 检查id是否在黑名单中
pub async fn is_in_black_list(
    id: &String,
    state: &web::Data<AppState>,
) -> Result<bool, error::Error> {
    // let result =
    //     lib::redis::has_key(format!("auth_black_list_{}", id), &state.redis, &state.log).await?;
...
// 撤销授权
pub async fn revoke_auth(id: u64, state: &web::Data<AppState>) -> Result<(), error::Error> {
