...
pub fn crypt_password(password: &str, salt: &Uuid) -> String {
...
pub async fn create_auth(
    ...
    let refresh_token_jti = Uuid::new_v4();
    ...
    let authorization_id =
        authorizations::service::create_auth(&authorization, client, state).await?;
...
pub fn create_access_token(user_id: u64, user_type: i16, config: &config::Config) -> Token {
...
pub async fn verify(
    ...
    req: &HttpRequest,
    state: &web::Data<AppState>,
) -> Result<AuthorizationInfo, error::Error> {
